<template>
  <header class="bg-white/90 dark:bg-gray-900/90 backdrop-blur-lg border-b border-gray-200/50 dark:border-gray-700/50 sticky top-0 z-40 shadow-lg transition-colors duration-300">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
      <div class="flex justify-between items-center h-20">
        ...
        <router-link class="flex items-center space-x-3 group" to="/" aria-label="Daily Journal">
          ...
          <div>
            <h1 class="text-xl font-bold bg-gradient-to-r from-gray-900 via-purple-800 to-blue-800 dark:from-gray-100 dark:via-purple-300 dark:to-blue-300 bg-clip-text text-transparent">
              Daily Journal
            </h1>
            <p class="text-xs text-gray-500 dark:text-gray-400 hidden sm:block font-medium">Reflect, Share & Grow</p>
          </div>
        </router-link>
        ...
        <button
          ...
          class="md:hidden p-2 rounded-lg text-gray-600 dark:text-gray-400 hover:text-gray-900 dark:hover:text-gray-100 hover:bg-gray-100 dark:hover:bg-gray-800 transition-colors duration-200"
        ...
        <div class="flex items-center space-x-2 sm:space-x-4">
          ...
          <button
            ...
            class="flex items-center space-x-2 sm:space-x-3 text-gray-700 dark:text-gray-300 hover:text-gray-900 dark:hover:text-gray-100 focus:outline-none group transition-all duration-200 p-2 rounded-xl hover:bg-gray-50 dark:hover:bg-gray-800"
          >
            <div class="relative">
            ...
            <div class="hidden sm:block text-left">
              <div class="text-sm font-semibold text-gray-900 dark:text-gray-100">{{ authStore.userDisplayName }}</div>
              <div class="text-xs text-gray-500 dark:text-gray-400">{{ authStore.user?.email }}</div>
            </div>
            <ChevronDownIcon class="w-4 h-4 text-gray-400 dark:text-gray-500 group-hover:text-gray-600 dark:group-hover:text-gray-300 transition-all duration-200" :class="showUserMenu ? 'rotate-180' : ''" />
          </button>

          <!-- Dropdown Menu -->
          <div
            v-if="showUserMenu"
            class="absolute right-0 mt-3 w-56 max-w-[calc(100vw-1rem)] bg-white/95 dark:bg-gray-800/95 backdrop-blur-md rounded-2xl shadow-xl border border-gray-200/50 dark:border-gray-700/50 py-2 z-50 fade-in"
          >
            <div class="px-4 py-3 border-b border-gray-100 dark:border-gray-700">
              <p class="text-sm font-medium text-gray-900 dark:text-gray-100">{{ authStore.userDisplayName }}</p>
              <p class="text-xs text-gray-500 dark:text-gray-400 truncate">{{ authStore.user?.email }}</p>
            </div>

            <router-link
              to="/"
              class="w-full text-left px-4 py-3 text-sm text-gray-700 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-gray-700 hover:text-gray-900 dark:hover:text-gray-100 flex items-center transition-colors duration-150"
            >
              <UserIcon class="w-4 h-4 mr-3 text-gray-400 dark:text-gray-500" />
              Journal Home
            </router-link>
            ...
            <router-link
              to="/chat"
              @click="showUserMenu = false"
              class="w-full text-left px-4 py-3 text-sm text-gray-700 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-gray-700 hover:text-gray-900 dark:hover:text-gray-100 flex items-center transition-colors duration-150 relative"
            >
              <svg class="w-4 h-4 mr-3 text-gray-400 dark:text-gray-500" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 10h.01M12 10h.01M16 10h.01M9 7a4 4 0 118 0v0a5.5 5.5 0 01-5.5 5.5v0H9a4 4 0 01-4-4v0z"/>
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17 21v-2a4 4 0 00-4-4H5a4 4 0 00-4 4v2"/>
            ...
            <div class="px-4 py-2 border-t border-gray-100 dark:border-gray-700">
              <p class="text-xs font-medium text-gray-500 dark:text-gray-400 mb-3">Connect with Developer</p>
              <div class="flex justify-center space-x-4">
                ...
                <a 
                  ...
                  target="_blank" 
                  rel="noopener noreferrer"
                  class="text-blue-700 dark:text-blue-400 hover:text-blue-900 dark:hover:text-blue-300 transition-colors duration-200"
                  title="LinkedIn"
                >
                  <svg class="w-5 h-5" fill="currentColor" viewBox="0 0 20 20">
            ...
            <button
              ...
              class="w-full text-left px-4 py-3 text-sm text-gray-700 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-gray-700 hover:text-gray-900 dark:hover:text-gray-100 flex items-center transition-colors duration-150"
            >
              <ArrowRightOnRectangleIcon class="w-4 h-4 mr-3 text-gray-400 dark:text-gray-500" />
              Sign out
            </button>
          </div>
        </div>
    ...
    <div
      ...
      class="md:hidden bg-white/95 dark:bg-gray-800/95 backdrop-blur-md border-t border-gray-200/50 dark:border-gray-700/50 shadow-lg"
    >
      <div class="px-4 py-3 space-y-2">
        ...
        <router-link
          to="/profile"
          @click="showMobileMenu = false"
          class="flex items-center px-4 py-3 rounded-xl text-sm font-medium text-gray-600 dark:text-gray-400 hover:text-gray-900 dark:hover:text-gray-100 hover:bg-gray-100 dark:hover:bg-gray-800 transition-all duration-200"
        >
          <svg class="w-5 h-5 mr-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M10.325 4.317c.426-1.756 2.924-1.756 3.35 0a1.724 1.724 0 002.573 1.066c1.543-.94 3.31.826 2.37 2.37a1.724 1.724 0 001.065 2.572c1.756.426 1.756 2.924 0 3.35a1.724 1.724 0 00-1.066 2.573c.94 1.543-.826 3.31-2.37 2.37a1.724 1.724 0 00-2.572 1.065c-.426 1.756-2.924 1.756-3.35 0a1.724 1.724 0 00-2.573-1.066c-1.543.94-3.31-.826-2.37-2.37a1.724 1.724 0 00-1.065-2.572c-1.756-.426-1.756-2.924 0-3.35a1.724 1.724 0 001.066-2.573c-.94-1.543.826-3.31 2.37-2.37.996.608 2.296.07 2.572-1.065z"></path>
            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"></path>
        ...
        <button
          @click="handleLogout"
          class="flex items-center w-full text-left px-4 py-3 rounded-xl text-sm font-medium text-red-600 dark:text-red-400 hover:text-red-700 dark:hover:text-red-300 hover:bg-red-50 dark:hover:bg-red-900/30 transition-all duration-200"
        >
    ...
    >
      <div class="bg-white dark:bg-gray-800 rounded-2xl max-w-md w-full p-8 shadow-2xl slide-up">
        <div class="text-center mb-6">
          ...
          <h3 class="text-xl font-bold text-gray-900 dark:text-gray-100">Profile Settings</h3>
        </div>
        
        <form @submit.prevent="handleUpdateProfile" class="space-y-6">
          <div>
            <label for="profile-name" class="block text-sm font-semibold text-gray-700 dark:text-gray-300 mb-2">
              Display Name
            </label>
            <input
              id="profile-name"
              ...
              type="text"
              class="w-full px-4 py-3 border border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-gray-100 rounded-xl focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-all duration-200"
          ...
          <div>
            ...
            <div class="text-sm text-gray-600 dark:text-gray-300 bg-gray-50 dark:bg-gray-700 p-3 rounded-xl border border-gray-200 dark:border-gray-600">
              {{ authStore.user?.email }}
            </div>
            <p class="text-xs text-gray-500 dark:text-gray-400 mt-2">Email cannot be changed</p>
          </div>

          <div v-if="profileError" class="p-4 bg-red-50 dark:bg-red-900/30 border border-red-200 dark:border-red-700 rounded-xl text-red-700 dark:text-red-300 text-sm">
            {{ profileError }}
          </div>

          <div v-if="profileSuccess" class="p-4 bg-green-50 dark:bg-green-900/30 border border-green-200 dark:border-green-700 rounded-xl text-green-700 dark:text-green-300 text-sm">
          ...
          <div class="flex gap-3 pt-4">
            <button
              type="button"
              @click="closeProfile"
              class="flex-1 bg-gray-100 dark:bg-gray-700 hover:bg-gray-200 dark:hover:bg-gray-600 text-gray-700 dark:text-gray-300 font-medium py-3 px-6 rounded-xl transition-all duration-200"
            >
              Cancel
            </button>
            <button
