<template>
  <div class="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-200/60 dark:border-gray-700/60 overflow-hidden hover:shadow-lg transition-all duration-300 transform hover:-translate-y-0.5 backdrop-blur-sm group">
    <!-- Enhanced User Header -->
    <div class="px-4 sm:px-6 py-3 sm:py-5 border-b border-gray-100/80 dark:border-gray-700/80 bg-gradient-to-r from-blue-50/30 via-purple-50/20 to-pink-50/30 dark:from-blue-900/20 dark:via-purple-900/10 dark:to-pink-900/20">
      <div class="flex items-center justify-between">
        <div class="flex items-center">
          ...
          <div class="ml-3 sm:ml-4">
            <p class="text-sm font-semibold text-gray-900 dark:text-gray-100 group-hover:text-blue-700 dark:group-hover:text-blue-300 transition-colors">{{ entry.userDisplayName }}</p>
        ...
        <div class="flex items-center space-x-2">
          <span class="inline-flex items-center px-2 sm:px-3 py-1 sm:py-1.5 rounded-full text-xs font-semibold bg-emerald-100 dark:bg-emerald-900/50 text-emerald-800 dark:text-emerald-300 border border-emerald-200/50 dark:border-emerald-700/50 shadow-sm">
    ...
    <div class="px-4 sm:px-6 py-4 sm:py-6">
      <h3 class="text-lg sm:text-xl font-bold text-gray-900 dark:text-gray-100 mb-4 sm:mb-6 flex items-center">
      ...
      <div class="space-y-4 sm:space-y-5">
        ...
        <div v-if="entry.gratitude" class="group-hover-section">
          <div class="bg-gradient-to-r from-amber-50/70 via-orange-50/50 to-yellow-50/60 dark:from-amber-900/30 dark:via-orange-900/20 dark:to-yellow-900/30 border-l-4 border-amber-400 dark:border-amber-500 p-4 sm:p-5 rounded-r-xl shadow-sm hover:shadow-md transition-all duration-300 hover:bg-gradient-to-r hover:from-amber-50 hover:via-orange-50 hover:to-yellow-50 dark:hover:from-amber-900/40 dark:hover:via-orange-900/30 dark:hover:to-yellow-900/40">
            <h4 class="font-semibold text-amber-900 dark:text-amber-100 text-sm mb-3 flex items-center">
            ...
            <p v-if="!showFullContent && entry.gratitude.length > 120" class="text-amber-800 dark:text-amber-200 text-sm sm:text-base leading-relaxed">
              {{ entry.gratitude.substring(0, 120) }}...
              <button 
                @click="showFullContent = true"
                class="text-blue-600 hover:text-blue-700 dark:text-blue-400 dark:hover:text-blue-300 font-medium ml-2 underline decoration-2 underline-offset-2 hover:decoration-blue-300 transition-all"
              >
                Read more
              </button>
            </p>
            <p v-else class="whitespace-pre-wrap text-amber-800 dark:text-amber-200 text-sm sm:text-base leading-relaxed">{{ entry.gratitude }}</p>
          </div>
        </div>

        <!-- Enhanced Emotion -->
        <div v-if="entry.emotion" class="group-hover-section">
          <div class="bg-gradient-to-r from-purple-50/70 via-pink-50/50 to-indigo-50/60 dark:from-purple-900/30 dark:via-pink-900/20 dark:to-indigo-900/30 border-l-4 border-purple-400 dark:border-purple-500 p-4 sm:p-5 rounded-r-xl shadow-sm hover:shadow-md transition-all duration-300 hover:bg-gradient-to-r hover:from-purple-50 hover:via-pink-50 hover:to-indigo-50 dark:hover:from-purple-900/40 dark:hover:via-pink-900/30 dark:hover:to-indigo-900/40">
            <h4 class="font-semibold text-purple-900 dark:text-purple-100 text-sm mb-3 flex items-center">
              <span class="text-xl sm:text-2xl mr-2.5 drop-shadow-sm">💭</span>
              <span class="uppercase tracking-wide text-xs sm:text-sm font-bold">Current Emotion</span>
            </h4>
            <p v-if="!showFullContent && entry.emotion.length > 120" class="text-purple-800 dark:text-purple-200 text-sm sm:text-base leading-relaxed">
            ...
            <p v-else class="whitespace-pre-wrap text-purple-800 dark:text-purple-200 text-sm sm:text-base leading-relaxed">{{ entry.emotion }}</p>
          </div>
        </div>

        <!-- Enhanced Challenges (if expanded) -->
        <div v-if="showFullContent && entry.challenges" class="group-hover-section">
          <div class="bg-gradient-to-r from-red-50/70 via-pink-50/50 to-rose-50/60 dark:from-red-900/30 dark:via-pink-900/20 dark:to-rose-900/30 border-l-4 border-red-400 dark:border-red-500 p-4 sm:p-5 rounded-r-xl shadow-sm hover:shadow-md transition-all duration-300 hover:bg-gradient-to-r hover:from-red-50 hover:via-pink-50 hover:to-rose-50 dark:hover:from-red-900/40 dark:hover:via-pink-900/30 dark:hover:to-rose-900/40">
            <h4 class="font-semibold text-red-900 dark:text-red-100 text-sm mb-3 flex items-center">
              <span class="text-xl sm:text-2xl mr-2.5 drop-shadow-sm">🎯</span>
              <span class="uppercase tracking-wide text-xs sm:text-sm font-bold">Challenges</span>
            </h4>
            <p class="whitespace-pre-wrap text-red-800 dark:text-red-200 text-sm sm:text-base leading-relaxed">{{ entry.challenges }}</p>
          </div>
        </div>

        <!-- Enhanced Learning (if expanded) -->
        <div v-if="showFullContent && entry.learning" class="group-hover-section">
          <div class="bg-gradient-to-r from-green-50/70 via-emerald-50/50 to-teal-50/60 dark:from-green-900/30 dark:via-emerald-900/20 dark:to-teal-900/30 border-l-4 border-green-400 dark:border-green-500 p-4 sm:p-5 rounded-r-xl shadow-sm hover:shadow-md transition-all duration-300 hover:bg-gradient-to-r hover:from-green-50 hover:via-emerald-50 hover:to-teal-50 dark:hover:from-green-900/40 dark:hover:via-emerald-900/30 dark:hover:to-teal-900/40">
            <h4 class="font-semibold text-green-900 dark:text-green-100 text-sm mb-3 flex items-center">
              <span class="text-xl sm:text-2xl mr-2.5 drop-shadow-sm">📚</span>
              <span class="uppercase tracking-wide text-xs sm:text-sm font-bold">Learning</span>
            </h4>
            <p class="whitespace-pre-wrap text-green-800 dark:text-green-200 text-sm sm:text-base leading-relaxed">{{ entry.learning }}</p>
          </div>
        </div>
        ...
        <div v-if="!showFullContent && (entry.challenges || entry.learning)" class="text-center pt-3">
          <button 
            @click="showFullContent = true"
            class="group-hover-expand inline-flex items-center px-6 py-3 text-sm font-semibold text-blue-700 dark:text-blue-300 bg-gradient-to-r from-blue-50 to-indigo-50 dark:from-blue-900/30 dark:to-indigo-900/30 rounded-xl hover:from-blue-100 hover:to-indigo-100 dark:hover:from-blue-900/50 dark:hover:to-indigo-900/50 transition-all duration-200 border border-blue-200/50 dark:border-blue-700/50 shadow-sm hover:shadow-md hover:scale-[1.02]"
          >
    ...
    <div class="px-4 sm:px-6 py-4 sm:py-5 bg-gradient-to-r from-gray-50 via-blue-50/30 to-gray-50 dark:from-gray-800 dark:via-gray-700 dark:to-gray-800 border-t border-gray-200/60 dark:border-gray-600/60">
      <div class="flex justify-between items-center">
        <!-- Entry Stats -->
        <div class="flex items-center space-x-4 text-sm text-gray-600 dark:text-gray-300">
          <div class="flex items-center space-x-2 bg-white/80 dark:bg-gray-700/80 rounded-lg px-3 py-2 border border-gray-200/50 dark:border-gray-600/50 shadow-sm">
            <svg class="w-4 h-4 text-gray-500 dark:text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"/>
              <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z"/>
            </svg>
            <span class="font-semibold">{{ entry.views || 0 }}</span>
        ...
        <button
          @click="viewFullEntry"
          class="group inline-flex items-center px-4 sm:px-6 py-2.5 sm:py-3 text-sm sm:text-base font-semibold text-white bg-gradient-to-r from-blue-600 to-blue-700 hover:from-blue-700 hover:to-blue-800 border border-blue-600 dark:border-blue-500 rounded-xl shadow-md hover:shadow-lg focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500 dark:focus:ring-offset-gray-800 transition-all duration-200 transform hover:scale-[1.02] active:scale-[0.98]"
        >
          <span class="text-blue-100 mr-2 text-lg">📖</span>
    ...
    <div class="px-4 sm:px-6 py-3 sm:py-5 border-t border-gray-100 dark:border-gray-700">
        ...
        <div class="relative" ref="reactionDropdown">
          <button
            ...
            :class="userHasReacted 
              ...
              : 'bg-white dark:bg-gray-800 text-gray-700 dark:text-gray-300 border-gray-300 dark:border-gray-600 hover:bg-gray-50 dark:hover:bg-gray-700 hover:border-gray-400 dark:hover:border-gray-500 hover:shadow-gray-200 dark:hover:shadow-gray-800'"
          >
          ...
          <div v-if="showReactionDropdown" 
               class="absolute bottom-full left-0 mb-3 bg-white dark:bg-gray-800 rounded-xl shadow-xl border border-gray-200 dark:border-gray-600 z-30 w-[240px] sm:w-[320px] ml-2 sm:ml-0 reaction-dropdown">
            <div class="flex p-2 sm:p-3 justify-between items-center bg-gradient-to-r from-blue-50/50 to-purple-50/50 dark:from-blue-900/20 dark:to-purple-900/20">
              <button
                v-for="reaction in REACTION_TYPES"
                :key="reaction.value"
                @click="handleReactionClick(reaction.value)"
                class="p-1.5 sm:p-2.5 rounded-lg hover:bg-white/80 dark:hover:bg-gray-700/80 text-base sm:text-lg transition-all duration-150 hover:scale-110 active:scale-95 w-[32px] h-[32px] sm:w-[42px] sm:h-[42px] flex items-center justify-center flex-shrink-0 shadow-sm hover:shadow-md"
    ...
    <div v-if="entry.reactions && entry.reactions.length > 0" class="px-4 sm:px-6 py-2 sm:py-3 border-t border-gray-50 dark:border-gray-700 bg-gradient-to-r from-blue-50/30 to-purple-50/20 dark:from-blue-900/10 dark:to-purple-900/10">
      <div class="flex items-center space-x-2 sm:space-x-3 text-xs sm:text-sm text-gray-600 dark:text-gray-400">
        <span class="font-semibold text-gray-800 dark:text-gray-200 text-xs sm:text-sm">Reactions:</span>
        <div class="flex items-center space-x-1.5 sm:space-x-3 flex-wrap">
          <div v-for="(reactionGroup, reactionType) in groupedReactions" 
               :key="reactionType"
               class="inline-flex items-center space-x-1 sm:space-x-2 bg-white/70 dark:bg-gray-800/70 rounded-full px-2 sm:px-3 py-1 sm:py-1.5 border border-gray-200/50 dark:border-gray-600/50 shadow-sm hover:shadow-md transition-all duration-200 hover:bg-white dark:hover:bg-gray-700">
            <span class="text-sm sm:text-base">{{ getReactionEmoji(reactionType) }}</span>
            <span class="font-bold text-gray-800 dark:text-gray-200 text-xs sm:text-sm">{{ reactionGroup.length }}</span>
    ...
    <div v-if="entry.comments && entry.comments.length > 0" class="px-4 sm:px-6 py-2 sm:py-3"
         :class="entry.reactions && entry.reactions.length > 0 ? 'bg-gradient-to-r from-purple-50/20 to-pink-50/20 dark:from-purple-900/10 dark:to-pink-900/10' : 'border-t border-gray-50 dark:border-gray-700 bg-gradient-to-r from-green-50/30 to-blue-50/20 dark:from-green-900/10 dark:to-blue-900/10'">
      <div class="flex items-center space-x-2 sm:space-x-3 text-xs sm:text-sm text-gray-600 dark:text-gray-400">
        <span class="font-semibold text-gray-800 dark:text-gray-200 text-xs sm:text-sm">Comments:</span>
        <div class="flex items-center space-x-1 sm:space-x-2 bg-white/70 dark:bg-gray-800/70 rounded-full px-2 sm:px-3 py-1 sm:py-1.5 border border-gray-200/50 dark:border-gray-600/50 shadow-sm">
          <svg class="w-3 h-3 sm:w-4 sm:h-4 text-gray-500 dark:text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 12h.01M12 12h.01M16 12h.01M21 12c0 4.418-4.03 8-9 8a9.863 9.863 0 01-4.255-.949L3 20l1.395-3.72C3.512 15.042 3 13.574 3 12c0-4.418 4.03-8 9-8s9 3.582 9 8z"/>
          </svg>
          <span class="font-bold text-gray-800 dark:text-gray-200 text-xs sm:text-sm">{{ entry.comments.length }}</span>
          <span class="text-gray-600 dark:text-gray-400 font-medium text-xs sm:text-sm">{{ entry.comments.length === 1 ? 'comment' : 'comments' }}</span>
    ...
    <div v-if="showComments" class="border-t border-gray-100 dark:border-gray-700 bg-gradient-to-b from-gray-50/50 to-white dark:from-gray-800/50 dark:to-gray-800">
      <!-- Enhanced Add Comment Form -->
      <div class="px-4 sm:px-6 py-3 sm:py-5 bg-gradient-to-r from-gray-50/80 to-blue-50/30 dark:from-gray-800/80 dark:to-blue-900/30">
        <div class="flex space-x-2 sm:space-x-4">
          ...
          <div class="flex-1">
            <textarea
              ...
              rows="2"
              class="w-full text-xs sm:text-base border border-gray-300 dark:border-gray-600 rounded-lg sm:rounded-xl px-3 sm:px-4 py-2 sm:py-3 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent resize-none text-black dark:text-white shadow-sm hover:shadow-md transition-shadow bg-white/80 dark:bg-gray-800/80 backdrop-blur-sm"
      ...
      <div v-if="entry.comments && entry.comments.length > 0" class="px-4 sm:px-6 py-2 sm:py-4 space-y-3 sm:space-y-5">
        <div v-for="comment in sortedComments" :key="comment.id" class="flex space-x-2 sm:space-x-4 group-hover-comment">
          ...
          <div class="flex-1 min-w-0">
            <div class="bg-gradient-to-r from-gray-50 to-blue-50/50 dark:from-gray-700 dark:to-blue-800/50 rounded-lg sm:rounded-xl px-3 sm:px-4 py-2 sm:py-3 shadow-sm hover:shadow-md transition-all duration-200 hover:bg-gradient-to-r hover:from-gray-100 hover:to-blue-100/60 dark:hover:from-gray-600 dark:hover:to-blue-700/60">
              <div class="flex justify-between items-start mb-1 sm:mb-2">
                <p class="text-xs sm:text-sm font-semibold text-gray-900 dark:text-gray-100 truncate">{{ comment.userDisplayName }}</p>
                <div class="flex items-center space-x-2 sm:space-x-3 ml-2 sm:ml-3 flex-shrink-0">
                  <span class="text-xs text-gray-500 dark:text-gray-400 font-medium">{{ formatDate(comment.createdAt) }}</span>
                  <button
                    v-if="comment.userId === authStore.user?.uid"
                    @click="deleteComment(comment.id)"
                    class="text-xs text-red-500 hover:text-red-700 dark:text-red-400 dark:hover:text-red-300 font-medium hover:bg-red-50 dark:hover:bg-red-900/30 px-1.5 sm:px-2 py-0.5 sm:py-1 rounded transition-all duration-200"
                  >
                    Delete
                  </button>
                </div>
              </div>
              <p class="text-xs sm:text-sm text-gray-700 dark:text-gray-300 whitespace-pre-wrap break-words leading-relaxed">{{ comment.content }}</p>
            </div>
          </div>
        </div>
      </div>
