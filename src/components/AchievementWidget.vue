  ...
  <div class="bg-gradient-to-br from-indigo-50 to-purple-50 rounded-xl p-6 border border-indigo-100">
    <div class="flex items-center justify-between mb-6">
      ...
      <div class="flex items-center space-x-2">
        <span class="text-sm text-gray-600">Level {{ gamificationStore.currentLevel }}</span>
    ...
    <div class="mb-6">
      <div class="flex justify-between items-center mb-2">
        ...
        <span class="text-sm text-gray-600">{{ gamificationStore.xpToNextLevel }} XP to level {{ gamificationStore.currentLevel + 1 }}</span>
      </div>
      ...
      <div class="text-center mt-1">
        <span class="text-xs text-gray-500">{{ gamificationStore.userStats.xp }} XP</span>
      </div>
    </div>

    <!-- Recent Badges -->
    <div v-if="recentBadges.length > 0" class="mb-6">
      <h4 class="text-sm font-semibold text-gray-700 mb-3">Recent Achievements</h4>
      <div class="flex flex-wrap gap-2">
        <div 
          v-for="badge in recentBadges"
          :key="badge.id"
          class="flex items-center space-x-2 bg-white rounded-lg px-3 py-2 border shadow-sm"
        ...
        >
          ...
          <div>
            ...
            <div class="text-xs text-gray-600">+{{ badge.xpReward }} XP</div>
          </div>
        </div>
      </div>
    </div>
    ...
    <div>
      <h4 class="text-sm font-semibold text-gray-700 mb-3">Progress</h4>
      <div class="space-y-3 max-h-48 overflow-y-auto">
        <div 
          ...
          :class="achievement.isUnlocked ? 'bg-green-50 border border-green-200' : 'bg-gray-50 border border-gray-200'"
        >
          <!-- Icon -->
          <div 
          ...
          <div class="flex-1 min-w-0">
            <div class="flex items-center justify-between">
              <h5 
                ...
                :class="achievement.isUnlocked ? 'text-green-900' : 'text-gray-900'"
              >
                {{ achievement.name }}
              </h5>
              <div class="flex items-center space-x-2">
