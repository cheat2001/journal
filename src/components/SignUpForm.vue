...
const termsError = computed(() => {
  if (!showValidation.value) return ''
  if (!form.acceptTerms) return 'You must agree to the terms'
  return ''
})
...
function validateForm() {
  ...
  if (displayNameError.value || emailError.value || passwordError.value || confirmPasswordError.value || termsError.value) {
    return false
  }
  return true
}
...
<template>
  <div class="w-full">
    ...
    <form @submit.prevent="handleSignUp" class="space-y-6">
      ...
      >
        <LoadingSpinner v-if="authStore.loading" class="w-4 h-4 mr-2" />
