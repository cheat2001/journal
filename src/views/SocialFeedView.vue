<template>
  <div class="min-h-screen bg-gradient-to-b from-blue-50 to-white dark:from-gray-900 dark:to-gray-800 transition-colors duration-300">
    ...
    <div class="max-w-4xl mx-auto px-3 sm:px-4 py-6 sm:py-8">
      <!-- Error State -->
      <div v-if="socialStore.error" class="mb-6 bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800 rounded-lg p-4">
        <div class="flex">
          ...
          <div class="ml-3">
            ...
            <p class="text-sm text-red-700 dark:text-red-300 mt-1">{{ socialStore.error }}</p>
            <button 
              @click="retryLoad"
              class="mt-2 text-sm bg-red-100 dark:bg-red-800 hover:bg-red-200 dark:hover:bg-red-700 text-red-800 dark:text-red-200 px-3 py-1 rounded transition-colors"
            >
              Try Again
            </button>
          </div>
      ...
      <div v-else-if="!socialStore.loading && socialStore.publicEntries.length === 0" class="text-center py-12">
        ...
        <h3 class="mt-2 text-sm font-medium text-gray-900 dark:text-gray-100">No public entries yet</h3>
        <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">Start sharing your journal entries with the community!</p>
      </div>

      <!-- Feed Content -->
      <div v-else class="space-y-4 sm:space-y-6">
        <!-- Feed Stats -->
        <div v-if="socialStore.publicEntries.length > 0" class="bg-white dark:bg-gray-800 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700 p-3 sm:p-4">
          <div class="flex items-center justify-between text-xs sm:text-sm text-gray-600 dark:text-gray-400">
            ...
            <div v-if="socialStore.hasMore" class="flex items-center space-x-1 text-blue-600 dark:text-blue-400">
              <div class="w-1.5 h-1.5 sm:w-2 sm:h-2 bg-blue-400 dark:bg-blue-500 rounded-full animate-pulse"></div>
              <span class="hidden sm:inline">More available</span>
              <span class="sm:hidden">More</span>
            </div>
            <div v-else class="text-green-600 dark:text-green-400 text-xs sm:text-sm">
              ✓ All caught up!
            </div>
          </div>
        </div>
        ...
        <div v-if="socialStore.loadingMore" class="flex justify-center py-8">
          <div class="flex items-center space-x-3 text-blue-600 dark:text-blue-400 bg-blue-50 dark:bg-blue-900/20 px-6 py-3 rounded-full">
            <LoadingSpinner size="sm" />
            <span class="text-sm font-medium">Loading more stories...</span>
            <div class="flex space-x-1">
              <div class="w-2 h-2 bg-blue-400 dark:bg-blue-500 rounded-full animate-bounce"></div>
              <div class="w-2 h-2 bg-blue-400 dark:bg-blue-500 rounded-full animate-bounce" style="animation-delay: 0.1s"></div>
              <div class="w-2 h-2 bg-blue-400 dark:bg-blue-500 rounded-full animate-bounce" style="animation-delay: 0.2s"></div>
            </div>
          </div>
        </div>
        ...
        >
          <div class="w-8 h-8 border-2 border-gray-200 dark:border-gray-600 rounded-full flex items-center justify-center">
        ...
        <div v-if="!socialStore.hasMore && socialStore.publicEntries.length > 0" class="text-center py-8">
          <div class="bg-gradient-to-r from-blue-50 to-purple-50 dark:from-blue-900/20 dark:to-purple-900/20 rounded-lg p-6 mx-4">
            <div class="text-4xl mb-2">🎉</div>
            <p class="text-gray-600 dark:text-gray-400 font-medium mb-1">You've reached the end!</p>
            <p class="text-gray-500 dark:text-gray-500 text-sm">You've seen all the latest community stories</p>
          </div>
        </div>
      </div>
    </div>
    ...
    >
      <button
        ...
        @click="scrollToTop"
        class="fixed bottom-4 right-4 sm:bottom-6 sm:right-6 z-50 bg-blue-600 hover:bg-blue-700 dark:bg-blue-700 dark:hover:bg-blue-800 text-white p-3 sm:p-3 rounded-full shadow-lg hover:shadow-xl transition-all duration-200 group touch-manipulation"
      >
