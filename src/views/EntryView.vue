<template>
  <div class="min-h-screen bg-gradient-to-br from-blue-50 via-white to-purple-50 dark:from-gray-900 dark:via-slate-900 dark:to-indigo-950 pt-16 sm:pt-24 pb-12">
    <div class="max-w-4xl mx-auto px-3 sm:px-4 md:px-6 lg:px-8">
      ...
      <div v-else-if="!entry" class="text-center py-16">
        ...
        <h2 class="text-2xl font-bold text-gray-900 dark:text-gray-100 mb-4">Entry Not Found</h2>
        <p class="text-gray-600 dark:text-gray-400 mb-6">This entry doesn't exist or is private.</p>
      ...
      <div v-else-if="!entry.isPublic" class="text-center py-16">
        ...
        <h2 class="text-2xl font-bold text-gray-900 dark:text-gray-100 mb-4">Private Entry</h2>
        <p class="text-gray-600 dark:text-gray-400 mb-6">This entry is private and can only be viewed by the author.</p>
        <router-link to="/feed" class="inline-flex items-center px-4 py-2 border border-transparent text-sm font-medium rounded-md shadow-sm text-white bg-blue-600 hover:bg-blue-700">
          Browse Community Feed
        </router-link>
      </div>
      ...
      <div v-else class="space-y-4 sm:space-y-8">
        <!-- Entry Header -->
        <div class="bg-white/80 dark:bg-gray-800/80 backdrop-blur-sm rounded-xl sm:rounded-2xl shadow-xl border border-gray-100 dark:border-gray-700 p-4 sm:p-6 lg:p-8">
          <!-- Author Info -->
          <div class="flex items-center space-x-3 sm:space-x-4 mb-4 sm:mb-6">
            ...
            <div class="flex-1 min-w-0">
              <h3 class="text-sm sm:text-lg font-semibold text-gray-900 dark:text-gray-100 truncate">{{ entry.authorName || entry.userDisplayName || 'Unknown User' }}</h3>
              <p class="text-xs sm:text-sm text-gray-500 dark:text-gray-400">{{ formatDate(entry.date) }}</p>
            </div>
            <div class="flex-shrink-0">
              <span class="inline-flex items-center px-2 sm:px-3 py-1 rounded-full text-xs font-medium bg-green-100 dark:bg-green-900/50 text-green-800 dark:text-green-300">
          ...
          <!-- Entry Title -->
          <h1 class="text-xl sm:text-2xl lg:text-3xl font-bold text-gray-900 dark:text-gray-100 mb-4 sm:mb-6 leading-tight">{{ entry.title || `Journal Entry - ${formatDate(entry.date)}` }}</h1>

          <!-- Mood Display -->
          <div v-if="entry.mood" class="flex items-center space-x-3 mb-4 sm:mb-6 p-3 sm:p-4 bg-gray-50 dark:bg-gray-700/50 rounded-xl">
            <div class="text-xl sm:text-2xl">{{ entry.mood.emoji }}</div>
            <div>
              <p class="text-xs sm:text-sm font-medium text-gray-900 dark:text-gray-100">{{ entry.mood.label }}</p>
              <p class="text-xs text-gray-500 dark:text-gray-400">Mood when writing</p>
            </div>
          </div>

          <!-- Entry Content -->
          <div class="prose prose-sm sm:prose-lg max-w-none text-gray-800 dark:text-gray-200 leading-relaxed space-y-3 sm:space-y-4">
            <!-- Gratitude -->
            <div v-if="entry.gratitude">
              <h4 class="font-semibold text-gray-900 dark:text-gray-100 text-base sm:text-lg mb-2 flex items-center">
                <span class="text-lg sm:text-xl mr-2">🙏</span>
                Gratitude
              </h4>
              <p class="whitespace-pre-wrap text-gray-700 dark:text-gray-300 leading-relaxed text-sm sm:text-base">{{ entry.gratitude }}</p>
            </div>
            ...
            <div v-if="entry.emotion">
              ...
              <p class="whitespace-pre-wrap text-gray-700 dark:text-gray-300 leading-relaxed text-sm sm:text-base">{{ entry.emotion }}</p>
            </div>
            ...
            <div v-if="entry.challenges">
              ...
              <p class="whitespace-pre-wrap text-gray-700 dark:text-gray-300 leading-relaxed text-sm sm:text-base">{{ entry.challenges }}</p>
            </div>
            ...
            <div v-if="entry.learning">
              ...
              <p class="whitespace-pre-wrap text-gray-700 dark:text-gray-300 leading-relaxed text-sm sm:text-base">{{ entry.learning }}</p>
            </div>
            ...
            <div v-if="!entry.gratitude && !entry.emotion && !entry.challenges && !entry.learning && entry.content">
              <p class="whitespace-pre-wrap text-gray-700 dark:text-gray-300 leading-relaxed text-sm sm:text-base">{{ entry.content }}</p>
            </div>
            ...
            <div v-if="!entry.gratitude && !entry.emotion && !entry.challenges && !entry.learning && !entry.content" class="text-center py-6 sm:py-8">
              ...
              <p class="text-gray-500 dark:text-gray-400 text-sm">No content available for this entry</p>
            </div>
          </div>

          <!-- Entry Stats -->
          <div class="flex items-center justify-between pt-4 sm:pt-6 mt-4 sm:mt-6 border-t border-gray-200 dark:border-gray-600">
            <div class="flex items-center space-x-3 sm:space-x-6 text-xs sm:text-sm">
              <div class="flex items-center space-x-1 sm:space-x-2 text-gray-500 dark:text-gray-400">
        ...
        <SocialInteractions
          v-if="entry"
          :entry="entry"
          class="bg-white/80 dark:bg-gray-800/80 backdrop-blur-sm rounded-xl sm:rounded-2xl shadow-xl border border-gray-100 dark:border-gray-700"
        />

        <!-- Related Entries -->
        <div v-if="relatedEntries.length > 0" class="bg-white/80 dark:bg-gray-800/80 backdrop-blur-sm rounded-xl sm:rounded-2xl shadow-xl border border-gray-100 dark:border-gray-700 p-4 sm:p-6 lg:p-8">
          <h3 class="text-lg sm:text-xl font-bold text-gray-900 dark:text-gray-100 mb-4 sm:mb-6">More from {{ entry.authorName || entry.userDisplayName || 'this user' }}</h3>
          <div class="grid gap-3 sm:gap-4 sm:grid-cols-2">
            <router-link
              v-for="relatedEntry in relatedEntries"
              :key="relatedEntry.id"
              :to="`/entry/${relatedEntry.id}`"
              class="block p-3 sm:p-4 rounded-lg sm:rounded-xl border border-gray-200 dark:border-gray-600 hover:border-blue-300 dark:hover:border-blue-500 hover:shadow-md transition-all duration-200 bg-white dark:bg-gray-700"
            >
              <h4 class="font-semibold text-gray-900 dark:text-gray-100 mb-2 line-clamp-2 text-sm sm:text-base">{{ relatedEntry.title || `Entry from ${formatDate(relatedEntry.date)}` }}</h4>
              <p class="text-xs sm:text-sm text-gray-600 dark:text-gray-300 line-clamp-3 mb-2 sm:mb-3">{{ relatedEntry.content || relatedEntry.gratitude || 'No preview available' }}</p>
